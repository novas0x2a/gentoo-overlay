...
src_install() {
	...
	dosym "${dir}"/dropboxd /opt/bin/dropboxd

	insinto "${EPREFIX}"/usr/share
	doins -r icons
	dosym "${EPREFIX}"/usr/share/icons "${EPREFIX}/opt/dropbox-daemon/icons"
	make_desktop_entry dropboxd "Dropbox Daemon" dropboxstatus-logo
	insinto "${EPREFIX}"/etc/xdg/autostart
	doins ${ED}/usr/share/applications/dropboxd-dropbox.desktop
}
